...
for($no=1000; $no<5999; $no++) {
	...
	print("<td>{$m[name]}</td>");
	print("<td>{$m[level]}</td>");
	print("<td><img src=\"$img_f{$m[img]}\">$img_f{$m[img]}</td>");
	print("<td>{$m[exphold]}</td>");
	print("<td>{$m[moneyhold]}</td>");
	print("<td>{$m[hp]}/{$m[maxhp]}</td>");
	print("<td>{$m[sp]}/{$m[maxsp]}</td>");
	print("<td>{$m[atk][0]}<br />{$m[atk][1]}</td>");
	print("<td>{$m[def][0]}+{$m[def][1]}<br />{$m[def][2]}+{$m[def][3]}</td>");
	print("<td>{$m[str]} / {$m[int]} / {$m[dex]} / {$m[spd]} / {$m[luk]}</td>");
	if($m["posed"])
		print("<td>-</td>");
	else
		print("<td>{$m[position]}</td>");
	print("<td>{$m[guard]}</td>");
	print("</tr>\n");
